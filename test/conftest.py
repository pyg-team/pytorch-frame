...
def get_fake_tensor_frame() -> Callable:
    def _get_fake_tensor_frame(
        ...
        feat_dict = {
            ...
            torch.randn(
                size=(num_rows, len(col_names_dict[torch_frame.numerical])),
                device=device,
            ),
            torch_frame.multicategorical:
            MultiNestedTensor.from_tensor_mat([[
                torch.arange(random.randint(0, 10), device=device)
                for _ in range(
                    len(col_names_dict[torch_frame.multicategorical]))
            ] for _ in range(num_rows)]),
            torch_frame.text_embedded:
            MultiEmbeddingTensor.from_tensor_list([
                torch.randn(num_rows, random.randint(1, 5), device=device)
            ...
            MultiNestedTensor.from_tensor_mat([[
                torch.randn(random.randint(0, 10), device=device)
                for _ in range(
