...
def test_categorical_feature_encoder(encoder_cls_kwargs):
    ...
    stats_list = [
        ...
        for col_name in tensor_frame.col_names_dict[torch_frame.categorical]
    ]
    encoder = encoder_cls_kwargs[0](
        8,
        stats_list=stats_list,
        stype=torch_frame.categorical,
...
def test_numerical_feature_encoder(encoder_cls_kwargs):
    ...
    stats_list = [
        dataset.col_stats[col_name]
        for col_name in tensor_frame.col_names_dict[torch_frame.numerical]
    ]
    encoder = encoder_cls_kwargs[0](8, stats_list=stats_list,
                                    stype=torch_frame.numerical,
                                    **encoder_cls_kwargs[1])
...
def test_multicategorical_feature_encoder(encoder_cls_kwargs):
    ...
    stats_list = [
        dataset.col_stats[col_name]
        for col_name in tensor_frame.col_names_dict[torch_frame.multicategorical]
    ]
    encoder = encoder_cls_kwargs[0](
        8,
        stats_list=stats_list,
        stype=torch_frame.multicategorical,
        **encoder_cls_kwargs[1],
    )
...
def test_text_embedded_encoder():
    ...
    stats_list = [
        dataset.col_stats[col_name]
        for col_name in tensor_frame.col_names_dict[torch_frame.text_embedded]
    ]
    encoder = LinearEmbeddingEncoder(
        out_channels=out_channels,
        stats_list=stats_list,
        stype=torch_frame.text_embedded,
    )
    x_text = tensor_frame.feat_dict[torch_frame.text_embedded]
    x = encoder(x_text)
    assert x.shape == (
        num_rows,
        len(tensor_frame.col_names_dict[torch_frame.text_embedded]),
        out_channels,
    )
...
def test_text_tokenized_encoder():
    ...
    stats_list = [
        dataset.col_stats[col_name]
        for col_name in tensor_frame.col_names_dict[torch_frame.text_tokenized]
    ]
    model = RandomTextModel(
        text_emb_channels=text_emb_channels,
        num_cols=len(tensor_frame.col_names_dict[torch_frame.text_tokenized]),
    )
    encoder = LinearModelEncoder(
        out_channels=out_channels,
        stats_list=stats_list,
        stype=torch_frame.text_tokenized,
        in_channels=text_emb_channels,
        model=model,
    )
    x_text = tensor_frame.feat_dict[torch_frame.text_tokenized]
    x = encoder(x_text)
    assert x.shape == (
        num_rows,
        len(tensor_frame.col_names_dict[torch_frame.text_tokenized]),
        out_channels,
    )
