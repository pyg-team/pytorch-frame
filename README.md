...
* `Materialization` handles converting the raw `DataFrame` into a `TensorFrame` that is amenable to Pytorch-based training and modeling.
...
* `self.convs`: We create a two layers of `ExampleTransformer`. Each `ExampleTransformer` module takes in an embedding of size `[batch_size, num_cols, channels]` and outputs an embedding of the same size.
